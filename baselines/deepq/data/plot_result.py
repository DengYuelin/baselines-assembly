...
def plot_campare(fuzzy_path, none_fuzz_path):

    reward_fuzzy = np.load(fuzzy_path)
    reward_none_fuzzy = np.load(none_fuzz_path)
    plt.figure(figsize=(15, 15), dpi=100)
    plt.subplot(2, 1, 1)
    plt.title('DQN Without Knowledge')
    plt.plot(np.arange(len(reward_none_fuzzy) - 1), np.array(reward_none_fuzzy[1:]))
    plt.ylabel('Episode Reward')
    plt.xlabel('Episodes')
    ...
    plt.plot(np.arange(len(reward_fuzzy) - 1), np.array(reward_fuzzy[1:] * 10))
    plt.ylabel('Episode Reward')
    plt.xlabel('Episodes')
    plt.show()
